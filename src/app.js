...
function shell(content) {
  const navGroups = [
    { label:'PILOTER', items:[['today','home','Priorités'],['intelligence','brain','Analyses']] },
...
function today() {
  ...
  if (!isDemo) return `<div class="today today-live page-enter">
    <header class="minimal-head"><span>ANETO / PRIORITÉS</span><div class="brain-status"><i></i>${bootstrap.memoryEvents.length} événement${bootstrap.memoryEvents.length>1?'s':''} vérifiable${bootstrap.memoryEvents.length>1?'s':''} dans l’historique</div></header>
    <section class="home-brief"><p>Bonjour${firstName ? ` ${firstName}` : ''}.</p><h1>Voici ce qui mérite<br><em>une décision.</em></h1><div class="home-brief-meta"><span>${analysis.count} contenus observés</span><span>${transcriptCount} transcriptions exploitables</span><span>${clipCandidates.length} cuts candidats</span></div></section>
    <section class="daily daily-first"><div class="daily-title"><p>PRIORITÉ ÉDITORIALE</p><span>${recommendations.length ? `${recommendations.length} décision${recommendations.length>1?'s':''}` : 'Preuves insuffisantes'}</span></div><div class="recommendations">${recommendationList}</div><a class="home-analysis-link" href="/intelligence">Voir le raisonnement et le niveau de preuve ${icon('arrow',14)}</a></section>
    ${syncButton}${syncedLibrary}
    <footer class="quiet-footer"><span>Chaque recommandation sépare faits, inférences et hypothèses.</span><button data-view="memory">Voir l’historique ${icon('arrow',14)}</button></footer>
  </div>`
  return `<div class="today page-enter"><header class="minimal-head"><span>ANETO / AUJOURD’HUI</span><div class="brain-status"><i></i>Le cerveau a appris 128 nouvelles choses cette nuit</div></header>${syncButton}${syncedLibrary}<section class="intent"><p>Bonjour${firstName ? ` ${firstName}` : ''}.</p><h1>Que veux-tu accomplir<br><em>aujourd’hui ?</em></h1>${goals}</section><section class="daily"><div class="daily-title"><p>AUJOURD’HUI, JE RECOMMANDE</p><span>${recommendations.length} décision${recommendations.length>1?'s':''}</span></div><div class="recommendations">${recommendationList}</div></section><footer class="quiet-footer"><span>Rien ne sera publié sans ton accord.</span><button data-view="memory">Ce qu’Aneto a appris ${icon('arrow',14)}</button></footer></div>`
}
...
function intelligence() {
  ...
  if (!isDemo) {
    ...
    return `<div class="page intelligence intelligence-v2 page-enter">
      <header class="page-head intelligence-head"><div><span>ANALYSES / RAISONNEMENT ÉDITORIAL</span><h1>Ce que les données permettent de décider.</h1></div><div class="learning intelligence-state ${maturity}"><i></i><span>Qualité de la preuve<strong>${maturity==='bloqué'?'Bloquée':maturity==='partiel'?'Partielle':maturity==='initial'?'Initiale':'En apprentissage'}</strong></span></div></header>
      <section class="understanding"><span>CONCLUSION ACTUELLE</span><h2>${escapeHtml(understanding)}</h2><p>${transcriptCount ? `${transcriptCount} transcription${transcriptCount>1?'s':''} sur ${analysis.count} contenu${analysis.count>1?'s':''} alimente${transcriptCount>1?'nt':''} cette lecture. Une conclusion reste provisoire tant que la couverture ou la répétition est faible.` : 'Les chiffres décrivent la performance. Les transcriptions permettront d’expliquer les sujets, les histoires, les émotions et les passages qui la provoquent.'}</p><details class="evidence-guide"><summary>Comment Aneto qualifie une preuve</summary><div><span><b>FAIT</b> Valeur reçue d’une API ou citation exacte.</span><span><b>INFÉRENCE</b> Relation calculée entre plusieurs faits.</span><span><b>HYPOTHÈSE</b> Explication plausible qui doit être testée.</span></div></details></section>
...
function clips() {
  ...
  const marketStudy = clipMarketStudy ? `<section class="clip-market-study"><div class="section-label"><span>BENCHMARK ÉDITORIAL INTERNE</span><em>Ce que les données permettent — et ne permettent pas — d’affirmer</em></div><div class="clip-market-grid"><article><small>OPPORTUNITÉ</small><p>${escapeHtml(clipMarketStudy.opportunity)}</p></article><article><small>AUDIENCE À VISER</small><p>${escapeHtml(clipMarketStudy.audience)}</p></article><article><small>DIFFÉRENCIATION</small><p>${escapeHtml(clipMarketStudy.differentiation)}</p></article><article><small>SIGNAL OBSERVÉ</small><p>${escapeHtml(clipMarketStudy.marketSignal)}</p></article><article class="is-caution"><small>LIMITES DE LA PREUVE</small><p>${escapeHtml(clipMarketStudy.limits)}</p></article><article class="is-test"><small>PROCHAIN TEST DÉCISIF</small><p>${escapeHtml(clipMarketStudy.nextTest)}</p></article></div></section>` : ''
  return `<div class="page clips-page page-enter">
    ...
    <section class="clips-manifesto"><span>ANETO A DÉJÀ DÉRUSHÉ</span><h2>Tu ne cherches plus dans les vidéos.<br>Tu choisis quoi tester.</h2><p>Chaque cut part d’un passage réellement prononcé. Le classement croise force éditoriale et, lorsqu’elle existe, rétention mesurée. Il sert à ordonner le visionnage — jamais à prédire des vues.</p><details class="score-guide"><summary>Comment lire le score de cut ?</summary><div><p><strong>Sans rétention :</strong> 100 % texte — hook, contraste, expérience vécue, fait concret et durée.</p><p><strong>Avec rétention :</strong> 70 % force éditoriale, 20 % rétention relative, 10 % audience encore présente.</p><p><strong>Score IA :</strong> une seconde lecture contradictoire note hook, autonomie, tension, conversation et fidélité au verbatim.</p></div></details></section>
    ${!isDemo ? `<section class="ai-editorial ${state.enrichingClips?'is-working':''}"><div class="ai-editorial-mark">${icon('spark',20)}</div><div><small>OPENROUTER / COMITÉ ÉDITORIAL</small><strong>${aiClipCount ? `${aiClipCount} finaliste${aiClipCount>1?'s':''} challengé${aiClipCount>1?'s':''}` : 'Mettre les passages en concurrence.'}</strong><p>Une requête, quatre regards : rédaction, montage, audience et fact-checking. L’IA doit éliminer, signaler le risque, formuler un test mesurable et rester fidèle au verbatim. Les hashtags sont des hypothèses à tester, pas des tendances temps réel.</p>${state.clipAiMessage?`<em class="${state.clipAiTone==='error'?'is-error':''}" role="status">${escapeHtml(state.clipAiMessage)}</em>`:''}</div><button id="enrich-clips" ${state.enrichingClips?'disabled':''}>${state.enrichingClips?'1 requête en cours…':aiClipCount?'Rechallenger le lot suivant':'Confronter tous les passages'} ${icon(state.enrichingClips?'sync':'arrow',15)}</button></section>` : ''}
    ${marketStudy}
...
function clipCard(clip,index) {
  ...
  const scorecard = clip.scorecard ? `<div class="clip-scorecard"><small>LECTURE DU COMITÉ IA</small>${Object.entries({Hook:clip.scorecard.hook,Autonomie:clip.scorecard.autonomy,Tension:clip.scorecard.tension,Conversation:clip.scorecard.conversation,Fidélité:clip.scorecard.fidelity}).map(([label,value])=>`<span><em>${label}</em><i style="--score:${Number(value)*10}%"></i><b>${value}/10</b></span>`).join('')}</div>` : ''
  const editorialKit = hasEditorialKit ? `<div class="clip-market-angle"><small>ANGLE · ${escapeHtml(clip.targetAudience)}</small><p>${escapeHtml(clip.marketAngle)}</p>${clip.whyNow?`<em><strong>Pourquoi maintenant :</strong> ${escapeHtml(clip.whyNow)}</em>`:''}</div>${scorecard}${clip.risk?`<div class="clip-challenge"><small>CONTRE-ARGUMENT</small><p>${escapeHtml(clip.risk)}</p></div>`:''}${clip.testHypothesis?`<div class="clip-test"><small>TEST À MESURER</small><p>${escapeHtml(clip.testHypothesis)}</p></div>`:''}<div class="clip-caption"><small>TEXTE PRÊT À PUBLIER</small><p>${escapeHtml(clip.caption)}</p><div>${(clip.hashtags ?? []).map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')}</div><small class="hashtag-caveat">Hashtags de découvrabilité à tester · pas une tendance temps réel</small>${clip.platformFit?.length?`<em>${escapeHtml(clip.platformFit.join(' · '))}</em>`:''}</div>` : ''
