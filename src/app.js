...
function clips() {
  ...
  return `<div class="page clips-page page-enter">
    ...
    ${!isDemo ? `<section class="ai-editorial ${state.enrichingClips?'is-working':''}"><div class="ai-editorial-mark">${icon('spark',20)}</div><div><small>OPENROUTER / COMITÉ ÉDITORIAL</small><strong>${aiClipCount ? `${aiClipCount} finaliste${aiClipCount>1?'s':''} challengé${aiClipCount>1?'s':''}` : 'Mettre les passages en concurrence.'}</strong><p>Une requête courte par vidéo, jusqu’à quatre en parallèle. Chaque jury croise rédaction, montage, audience et fact-checking ; Aneto fusionne ensuite les finalistes sans nouvel appel IA. Une vidéo en échec ne bloque plus les autres.</p>${state.clipAiMessage?`<em class="${state.clipAiTone==='error'?'is-error':''}" role="status">${escapeHtml(state.clipAiMessage)}</em>`:''}</div><button id="enrich-clips" ${state.enrichingClips?'disabled':''}>${state.enrichingClips?'Analyses courtes en cours…':aiClipCount?'Analyser les vidéos suivantes':'Lancer les analyses courtes'} ${icon(state.enrichingClips?'sync':'arrow',15)}</button></section>` : ''}
    ${marketStudy}
